...
const { getUser, createUser, updateUser, deleteUser, getUserById, getUserStatistics } = require('../controllers/userController');
const { protect, isAdmin } = require('../middlewares/authMiddleware');

// Define routes
router.get('/', protect, isAdmin, getUser); // Get all users (CEO only)
router.get('/stats', protect, isAdmin, getUserStatistics); // Get user statistics (CEO only)
